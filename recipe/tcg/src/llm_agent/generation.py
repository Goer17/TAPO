...
import requests
from .geogpt_topkcommon import geogpt_batch_search
    ...
    def execute_predictions(self, predictions: List[str], pad_token: str, active_mask=None, do_search=True, do_code=True) -> List[str]:
        ...
        if do_search:
            if not search_queries:
                ...
                if self.config.search_engine_name == "geogpt":
                    search_results: List[str] = geogpt_batch_search(search_queries, self.config.topk)
                elif self.config.search_engine_name == "serper":
